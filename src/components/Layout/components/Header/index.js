...
function Header() {
    ...
    const [isMobileDropdownOpen, setIsMobileDropdownOpen] = useState(false);
    const [error, setError] = useState(null);
    const [isScrolled, setIsScrolled] = useState(false); // Thêm state mới để theo dõi scroll
    const navigate = useNavigate();

    // Lắng nghe sự kiện scroll
    useEffect(() => {
        const handleScroll = () => {
            if (window.scrollY > 50) { // Khi scroll quá 50px
                setIsScrolled(true);
            } else {
                setIsScrolled(false);
            }
    ...
    return (
        <div className={cx("wrapper", { scrolled: isScrolled })}> {/* Thêm class động */}
            <div className={cx("navbar")}>
                <div className={cx("nav-left")}>
                    <Link to={"/"} className={cx("logo-name")}>
                        <h2>Tăng Ca</h2>
                    ...
                    <ul>
                        ...
                        <Tippy 
                                ...
                                <div className={cx("country-list")}>
                                    {countries.map((country) => (
                                        <h5 
                                            key={country.id} 
                                            style={{ marginLeft: "10px", padding: "8px" }} 
                                            onClick={() => handleGenerateMovieByCountry(country.slug)}
                                        >
                                            {country.name}
                                        </h5>
                                    ))}
                            ...
                            placement="bottom" 
                            interactive 
                            visible={isVisibleAccount} 
                            onClickOutside={hideAccount}
                        >  
                            <div className={cx("country-btn")}>
                                <Link 
                                    className={cx("link")} 
                                    onClick={isVisibleAccount ? hideAccount : showAccount}
                                >
            ...
            {isMenuOpen && (
                <div className={cx("mobile-menu")}>
                    ...
                    <ul>
                        <Link to={"/"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>Trang Chủ</li>
                        </Link>
                        <Link to={"/phim-le"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>Phim Lẻ</li>
                        </Link>
                        <Link to={"/phim-bo"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>Phim Bộ</li>
                        </Link>
                        <Link to={"/anime"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>Anime</li>
                        </Link>
                        <Link to={"/tv-shows"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>TV-Shows</li>
                        </Link>
                        <Link to={"/saved-movie"} className={cx("link-mobile")} onClick={toggleMenu}>
                            <li>Phim Yêu Thích</li>
                        </Link>
                        <Tippy
                            render={(attrs) => (
